...
function createLocalizedLabelMap<T extends string>(
  ...
  return new Proxy({} as Record<T, string>, {
    get(_target, prop) {
      if (typeof prop !== 'string' || !keys.includes(prop as T)) return undefined
...
export const TaxContractStatusLabel = createLocalizedLabelMap<TaxContractStatus>(
  {
    ACTIVE: '有効',
    EXPIRED: '期限切れ',
    TERMINATED: '解約済み',
...
export const DepositTransactionTypeLabel = createLocalizedLabelMap<DepositTransactionType>(
  {
    RECHARGE: 'チャージ',
    OFFSET: '充当',
    REFUND: '返金',
...
export const StaffRelationTypeLabel = createLocalizedLabelMap<StaffRelationType>(
  {
    PRIMARY: '主担当',
    SECONDARY: '副担当',
    SUPPORT: 'サポート',
